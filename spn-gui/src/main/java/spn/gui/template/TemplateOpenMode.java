...
/**
 * Shown when opening a .spnt file. Asks whether to instantiate
 * the template (fill in placeholders) or edit it as plain text.
 */
public class TemplateOpenMode implements Mode {
    ...
    private static final String[] LABELS = { "Instantiate template", "Edit template source" };
    ...
    private int selected = 0; // default: instantiate

    public TemplateOpenMode(EditorWindow window, Path filePath, String content) {
        this.window = window;
    ...
    public void render(float width, float height) {
        ...
        font.drawText("Open: " + fileName, px, y, FONT_SCALE, TITLE_R, TITLE_G, TITLE_B);
        y += rowH + 16f;
    ...
    public String hudText() {
        return "1 Instantiate | 2 Edit | Esc Cancel";
    }

    private void execute(int option) {
        window.popMode(); // remove this dialog
        switch (option) {
            case 0 -> {
                // Instantiate: load into editor, then push instantiation mode
                window.loadTemplateForInstantiation(filePath, content);
            }
            case 1 -> {
                // Edit: load as plain text
                window.loadFileDirectly(filePath, content);
            }
        }
    }
